...
class CompressorContainer(type: ContainerType<*>, id: Int, playerInv: PlayerInventory, extraInv: IInventory = Inventory(2)) : FSContainer(type, id){
    object Factory: IContainerFactory<CompressorContainer> {
        override fun create(windowId: Int, inv: PlayerInventory, data: PacketBuffer) = CompressorContainer(Registries.compressorContainerType, windowId, inv, inv.player.world.getTileEntity(data.readBlockPos()) as? CompressorTileEntity ?: error(""))
    }
    init {
